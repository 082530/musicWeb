<template>
  <div class="search">
    ...
    <div class="type">
      <span :style="type==1?'color:#0576DA;font-weight:bold':''" @click="type=1;find()">歌</span><span :style="type==1004?'color:#0576DA;font-weight:bold':''" @click="type=1004;find()">MV</span>
    </div>
    <div class="list">
      <ul v-if="type==1">
  ...
  methods: {
    ...
    find: function () {
      ...
      $.get(this.url + '/search?keywords=' + that.str + '&offset=' + this.offset + '&type=' + this.type, function (data) {
        if (data.result.songs) { that.list = data.result.songs } else that.list = data.result.mvs
